...
ConfigParseToConsole::~ConfigParseToConsole()
{
  ...
  delete pPushLoopKeyword;
  delete pLoopKeyword;
  delete pPushTypeKeyword;
  delete pTypeKeyword;

  //delete pPushBracelessScope;
  //delete pBracelessScope;
...
Parser* ConfigParseToConsole::Build()
{
  try
  {
	...
	// configure to detect and act on type keywords like  "class", "struct", "enum", "namespace"
	...
	// configure to detect and act on loop keywords like  "for", "while", "switch", "if", "catch"
	pLoopKeyword = new LoopKeyword;
	pPushLoopKeyword = new PushLoopKeyword(pRepo);
	pLoopKeyword->addAction(pPushLoopKeyword);
	pParser->addRule(pLoopKeyword);
	//pBracelessScope = new BracelessScope;pPushBracelessScope = new PushBracelessScope(pRepo);pBracelessScope->addAction(pPushBracelessScope);//pParser->addRule(pBracelessScope);
...

std::vector<Node*> ConfigParseToConsole::getAnalysedScopes(){
	return pRepo->getanalysedScopes();
}


Tree* ConfigParseToConsole::getLargestTree(){
